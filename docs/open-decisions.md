...
### 13. メール送信方式（決定）
- **独自ドメイン + Resend（送れない場合はレンタルサーバーのメール）を採用**（3案のうち①）。Gmail SMTP / Gmail API 案は不採用（「ちゃんとした会社」イメージ重視）。
...
- Resend無料枠は 100通/日・500通/月。超えるならレンタルサーバー（インターサーバー等、ドメイン込み 月約2,600円）に切替。費用対効果で後日判断。
- 連絡先メールはドメイン取得後にそのドメインのアドレスへ（それまで暫定 Gmail）。SPF/DKIM はドメイン認証時に対応。
